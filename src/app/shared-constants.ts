...
// ─── Design Tokens ───
// THEME BRIDGE: surfaces + composites read CSS variables (defined in tokens.css,
// swapped per data-theme = classic|stock|glass). Classic values are identical to
// the prior hardcoded hex, so the default theme is a zero-regression baseline.
// Brand ACCENTS and TEXT stay hex on purpose — they're constant across themes and
// are concatenated with alpha suffixes in ~800 call-sites (e.g. D.amber + "1c"),
// which would break under var(). Surface alpha-concat sites were converted to
// color-mix() (see slop-top / sa-weekly / press-to-premier / morning-brief).
export var D = {
  ...
  glow: "var(--glow)",
  glowHover: "var(--glow-hover)",
  cardGrad: "var(--card-grad)",
  surfGrad: "var(--surf-grad)",
};
